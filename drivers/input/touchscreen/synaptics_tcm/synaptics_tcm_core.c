...
SHOW_STORE_PROTOTYPE(syna_tcm, range_changer)
SHOW_STORE_PROTOTYPE(syna_tcm, orientation_change)
...
static struct device_attribute *dynamic_config_attrs[] = {
	...
	ATTRIFY(cover_y_min),
	ATTRIFY(cover_y_max),
	ATTRIFY(range_changer),
	ATTRIFY(orientation_change),
#if defined(SOMC_TOUCH_BRINGUP)
	ATTRIFY(cover_mode),
	ATTRIFY(irq_enable),
#endif
...
int portrait_buffer_t[4] = {25, 150, 60, 200};
int landscape_buffer_t[4] = {150, 20, 200, 60};
...
static ssize_t syna_tcm_sysfs_range_changer_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	struct device *p_dev;
...
static ssize_t syna_tcm_sysfs_range_changer_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int input[4] = {0};
	int status = 0;
	...
	sscanf(buf, "%d %d %d %d %d", &status, &input[0], &input[1], &input[2], &input[3]);

	if (status == 1)
		memcpy(tcm_hcd->landscape_buffer, input, sizeof(landscape_buffer_t));
	else
		memcpy(tcm_hcd->portrait_buffer, input, sizeof(landscape_buffer_t));

	return count;
}

static ssize_t syna_tcm_sysfs_orientation_change_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	struct device *p_dev;
...
static ssize_t syna_tcm_sysfs_orientation_change_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	struct device *p_dev;
	...
	if (input == 1)
		tcm_hcd->orientation = true;
	else
		tcm_hcd->orientation = false;
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	LOGD(tcm_hcd->pdev->dev.parent,
					"Unlock Power Ok\n");
#endif
	memcpy(tcm_hcd->landscape_buffer, landscape_buffer_t, sizeof(landscape_buffer_t));
	memcpy(tcm_hcd->portrait_buffer, portrait_buffer_t, sizeof(landscape_buffer_t));
