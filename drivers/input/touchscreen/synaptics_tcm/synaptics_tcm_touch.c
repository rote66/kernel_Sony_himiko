...
bool exist_flag[10] = {false};
bool report_flag[10] = {false};
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		case LIFT:
			...
			input_mt_report_slot_state(touch_hcd->input_dev,
					MT_TOOL_FINGER, 0);
#endif
			exist_flag[idx] = false; //relase
			report_flag[idx] = false;
			break;
		case FINGER:
		case GLOVED_FINGER:
			...
			if (tcm_hcd->orientation) {
				if (!exist_flag[idx]) { // press
					exist_flag[idx] = true;
					if ((x < tcm_hcd->landscape_buffer[0] || x > touch_hcd->input_params.max_x - tcm_hcd->landscape_buffer[0]) && (y < tcm_hcd->landscape_buffer[1] || y > touch_hcd->input_params.max_y - tcm_hcd->landscape_buffer[1])) {
						if (!report_flag[idx])
							goto skip_process;
					} else
						report_flag[idx] = true;
				} else { // move
					if ((x < tcm_hcd->landscape_buffer[0] || x > touch_hcd->input_params.max_x - tcm_hcd->landscape_buffer[0]) && (y < tcm_hcd->landscape_buffer[1] || y > touch_hcd->input_params.max_y - tcm_hcd->landscape_buffer[1])) {
						if (!report_flag[idx])
							goto skip_process;
					} else if ((x > tcm_hcd->landscape_buffer[2] && x < touch_hcd->input_params.max_x - tcm_hcd->landscape_buffer[2]) || (y > tcm_hcd->landscape_buffer[3] && y < touch_hcd->input_params.max_y - tcm_hcd->landscape_buffer[3])) {
						if (!report_flag[idx])
							report_flag[idx] = true;
					} else if (!report_flag[idx])
						goto skip_process;
				}
			} else {
				if (!exist_flag[idx]) {
					exist_flag[idx] = true;
					if ((x < tcm_hcd->portrait_buffer[0] || x > touch_hcd->input_params.max_x - tcm_hcd->portrait_buffer[0]) && (y > touch_hcd->input_params.max_y - tcm_hcd->portrait_buffer[1])) {
						if (!report_flag[idx])
							goto skip_process;
					} else
						report_flag[idx] = true;
					...
					if ((x < tcm_hcd->portrait_buffer[0] || x > touch_hcd->input_params.max_x - tcm_hcd->portrait_buffer[0]) && (y > touch_hcd->input_params.max_y - tcm_hcd->portrait_buffer[1])) {
						if (!report_flag[idx])
							goto skip_process;
					} else if ((x > tcm_hcd->portrait_buffer[2] && x < touch_hcd->input_params.max_x - tcm_hcd->portrait_buffer[2]) || (y < touch_hcd->input_params.max_y - tcm_hcd->portrait_buffer[3])) {
						if (!report_flag[idx])
							report_flag[idx] = true;
					} else if (!report_flag[idx])
						goto skip_process;
